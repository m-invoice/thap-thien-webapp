...
function getComment(percent: number) {
  if (percent >= 90) {
    return 'Rất tốt. Anh nắm bài khá chắc.'
  }
  if (percent >= 70) {
    return 'Khá tốt. Chỉ cần ôn lại vài ý chưa vững.'
  }
  if (percent >= 50) {
    return 'Tạm ổn. Nên xem lại bài học và phần câu sai.'
  }
  return 'Cần ôn lại kỹ hơn. Đừng lo, học lại đúng chỗ sẽ tiến bộ rất nhanh.'
}
...
export default async function ResultPage({
    ...
    return (
      <main className="min-h-screen bg-black text-white p-8">
        <div className="max-w-2xl mx-auto">
          ...
          <a
            href="/dashboard"
            className="inline-block px-5 py-3 bg-white text-black rounded-lg font-semibold"
          >
            Về Dashboard
          </a>
        </div>
  ...
  const attempt: AttemptData = {
    ...
    topics: attemptRaw.topics
      ? {
          title: (attemptRaw.topics as any).title ?? 'Không rõ chủ đề',
          slug: (attemptRaw.topics as any).slug ?? '',
        }
      : null,
  }
  ...
  const wrongAnswers: WrongAnswerRow[] = ((wrongRaw as unknown[]) || []).map(
    (item: any) => ({
      id: item.id,
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="border border-white/20 rounded-2xl p-6 mb-8">
          <div className="grid md:grid-cols-3 gap-4 text-center">
            ...
            <div>
              <div className="text-white/60 text-sm">Tỷ lệ đúng</div>
              <div className="text-3xl font-bold mt-2">
                {percent}%
              </div>
            </div>
            ...
            <div>
              <div className="text-white/60 text-sm">Câu cần ôn lại</div>
              <div className="text-3xl font-bold mt-2">
                {reviewCount}
              </div>
            </div>
        ...
        <div className="flex justify-center gap-4 flex-wrap mb-10">
          <a
            href="/dashboard"
            className="px-5 py-3 bg-white text-black rounded-lg font-semibold"
          >
            Về Dashboard
          </a>
        ...
        <div className="border border-white/20 rounded-2xl p-6">
          ...
          {wrongAnswers.length === 0 ? (
            <div className="border border-green-400/20 rounded-xl p-5 text-green-300">
              ...
              {wrongAnswers.map((item, index) => (
                ...
                >
                  ...
                  <div className="mt-4 text-red-400">
                    Anh đã chọn: {item.selected_option || 'Chưa chọn'}
                  </div>

                  <div className="mt-2 text-green-400">
                    Đáp án đúng: {item.correct_option || 'Không rõ'}
                  </div>

                  {item.questions?.explanation && (
                    <div className="mt-4 text-white/70">
                      <span className="font-semibold text-white">Lời giải:</span>{' '}
                      {item.questions.explanation}
                    </div>
                  )}
                </div>
